...
def Algo_Interesante():
    ...
    data = pd.read_csv(subprocess.run(
        ["netsh", "wlan", "show", "profiles"], capture_output=True).stdout.decode(), encoding='unicode_escape')
    arquivo_salva = subprocess.run(
        ["type", "nul", ">", "minhas_senhas.txt"], capture_output=True).stdout.decode()

    nomes_do_perfil = (re.findall(
        "Todos os Perfis de Usuários: (.*)\r", data))
    lista_wifi = []
    if len(nomes_do_perfil) != 0:
        for nome in nomes_do_perfil:
            ...
            if re.search("Chave de segurança           : Ausente", informacao_perfil):
                ...
                objPerfil = Senha_perfil(
                    perfil_wifi["ssid"], perfil_wifi["password"])
